...
private struct TerminalAppKitView: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> TerminalScrollView {
        ...
        view.terminalTextView.sendBytes = { [weak session] data in
            Task { @MainActor in session?.send(data) }
        }
        view.onResize = { [weak session] columns, rows in
            Task { @MainActor in session?.resize(columns: columns, rows: rows) }
        }
        return view
    }
    ...
    func updateNSView(_ nsView: TerminalScrollView, context: Context) {
        ...
        nsView.terminalTextView.sendBytes = { [weak session] data in
            Task { @MainActor in session?.send(data) }
        }
        nsView.onResize = { [weak session] columns, rows in
            Task { @MainActor in session?.resize(columns: columns, rows: rows) }
        }
    }
}
    ...
    override func keyDown(with event: NSEvent) {
        ...
        if flags.contains(.control),
           ...
           scalar.value >= 0x40, scalar.value <= 0x7F {
            sendBytes?(Data([UInt8(scalar.value & 0x1F)]))
            return
        }
        ...
        default:
            if let characters = event.characters, !characters.isEmpty {
                sendBytes?(Data(characters.utf8))
            }
        }
    }
